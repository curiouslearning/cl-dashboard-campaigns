WITH
  ...
  SELECT
    ...
    WHERE
      ep.key = 'cr_user_id') AS cr_user_id,
    -- campaign_id
    (
    SELECT
    ...
    LOWER(REGEXP_EXTRACT( (
        ...
        FROM
          UNNEST(event_params) ep
        WHERE
          ep.key = 'web_app_url'), r'[?&]cr_lang=([^&]+)') ) AS app_language,
  ...
  WHERE
    _TABLE_SUFFIX BETWEEN '20241108'
    ...
    FROM
      UNNEST(event_params) ep
    WHERE
      ep.key = 'web_app_url') LIKE 'https://feedthemonster.curiouscontent.org%'
