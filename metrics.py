...
def get_totals_by_metric(df,
    ...
    df_user_list = filter_user_data(df,
        daterange, countries_list, stat, language=language
    )
...
def filter_user_data(df,
    ...
    if not all(key in st.session_state for key in ["df_campaign_users", ]) or len(df) == 0:
        print("PROBLEM!")
        return pd.DataFrame()
